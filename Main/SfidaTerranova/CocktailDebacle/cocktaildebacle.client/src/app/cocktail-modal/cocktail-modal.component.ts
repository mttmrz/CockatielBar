...
export class CocktailModalComponent implements OnInit {
  ...
  isCreatedCocktail:boolean = false;
  CocktailIcon:string = "";
...
async ngOnInit(): Promise<void> {
  ...
  async checkOwner()
  {
    if (this.cocktail)
      {
      if (this.cocktail.userIdCocktail !== 0)
      {
       let cU = this.UserService.getUser()  
        if (cU)
        {
          if (cU.id === this.cocktail.userIdCocktail)
          {
            this.isCreatedCocktail = true;
          }
          else
          {
            this.isCreatedCocktail = false
          }
        }
      }
      else
...
saveCocktail(id:number) {
    ...
    console.log("LA NUOVA", new_Cocktail)
    this.CocktailService.updateCocktail(new_Cocktail, id);
    setTimeout(() => {
      this.created.emit()
      this.onClose()
    }, 1000);
